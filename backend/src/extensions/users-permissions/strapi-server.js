...
const issueRefreshToken = (payload, stayLoggedIn, jwtOptions = {}) => {
    _.defaults(jwtOptions, strapi.config.get('plugin.users-permissions.jwt'));
    const expiresIn = stayLoggedIn ? process.env.LONG_REFRESH_TOKEN_EXPIRES : process.env.REFRESH_TOKEN_EXPIRES;
    return jwt.sign(
        _.clone(payload.toJSON ? payload.toJSON() : payload),
...
module.exports = (plugin) => {
    ...
    plugin.controllers.auth.callback = async (ctx) => {
        ...
        if (provider === 'local') {
            ...
            const { identifier,stayLoggedIn } = params;
            ...
            if (!validPassword) {
                ...
                ctx.cookies.set("refreshToken", issueRefreshToken({ id: user.id },stayLoggedIn), {
                    httpOnly: true,
                    secure: false,
                    signed: true,
                    overwrite: true,
